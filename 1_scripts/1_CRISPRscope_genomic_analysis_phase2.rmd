...
```{r}
results_folder <- "../0_data/CRISPRscope_results"      # Not gitted. Contains .rds results from the pipeline and diverse small intermediary results.
...
write.table(unique_species, file = paste(repository, "/0_data/Metaphlan_results/metaphlan_unique_species.csv", sep=""), row.names = F, col.names = F, quote = F, eol = "\n")
...
species_colors <- read_delim(paste(repository,"/0_data/IMPORT/species_colors.txt", sep = ""),
...
X20210428_metaphlan_Foodbionet_final_species <- 
  read_csv(
    paste(
      repository,"/0_data/IMPORT/20210428_metaphlan_final_species.txt", sep = "")) %>% 
...
Clusters_CRISPRscope_spacers80 <- read_csv(paste(
      repository,"/0_data/IMPORT/clustering/Clusters0.8_CRISPRscope_genomic_185_spacers.csv", sep = "")) %>% 
  distinct(seq, cluster, identity)

Clusters_CRISPRscope_repeats80 <- read_csv(paste(
      repository,"/0_data/IMPORT/clustering/Clusters0.8_CRISPRscope_genomic_185_repeats.csv", sep = "")) %>% 
  distinct(seq, cluster, identity)
...
Clusters_CRISPRscope_spacers_identity <- read_csv(paste(
      repository,"/0_data/IMPORT/clustering/Clusters1_CRISPRscope_ALL_SPACERS.csv", sep = "")) %>% 
  distinct(seq, cluster, identity) %>% select(-identity)

Clusters_CRISPRscope_repeats_identity <- read_csv(paste(
      repository,"/0_data/IMPORT/clustering/Clusters1_CRISPRscope_ALL_REPEATS.csv", sep = "")) %>% 
  distinct(seq, cluster, identity) %>% select(-identity)
...
# Genome count. Original number of genomes from NCBI / Dialact
  ...
  genome_count_SELDIALACT <- read_delim(paste(repository, "/0_data/CRISPRscope_results/genome_count_SELDIALACT.txt", sep=""), 
    ";", escape_double = FALSE, trim_ws = TRUE) %>% mutate(count = `12`, Species = X2) %>% select(-`12`, -X2)
  genome_count_SELDIALACT %>% write.csv(paste(repository, "/0_data/CRISPRscope_results/export_genome_count_SELDIALACT.csv", sep=""))

  genome_count_SEL185 <- read_delim(paste(repository, "/0_data/CRISPRscope_results/genome_count_SEL185.txt", sep=""), 
    ";", escape_double = FALSE, trim_ws = TRUE) %>% mutate(count = `185`, Species = X2) %>% select(-`185`, -X2)
  genome_count_SEL185 %>% write.csv(paste(repository, "/0_data/CRISPRscope_results/export_genome_count_SEL185.csv", sep=""))

  genome_count_TOTAL <- bind_rows(genome_count_SEL185, genome_count_SELDIALACT)
  genome_count_TOTAL %>% write.csv(paste(repository, "/0_data/CRISPRscope_results/export_genome_count_total.csv", sep=""))
...
for (f in list.files(path = paste(repository, "/0_data/GCcontent/OUTPUT", sep=""))){

  strain <- str_replace(f,"_gc_content.tsv$", "")
  
  
  gc_mean <- read_delim(file = paste(repository, "/0_data/GCcontent/OUTPUT/",f, sep=""), col_names = c("name", "gc"), delim = "\t") %>% 
...
saveRDS(CRISPRscope_tbl_185, file = paste(repository, "/0_data/IMPORT_EXPORT/export_complete_SEL185.rds", sep=""))
saveRDS(CRISPRscope_tbl_26, file = paste(repository, "/0_data/IMPORT_EXPORT/export_complete_SEL26.rds", sep=""))
```
...
# Genomes with CRISPR / Species without CRISPR 
  ...
  species_without_crispr %>% write.csv(file = paste(repository, "/0_data/CRISPRscope_results/species_without_crispr.csv", sep=""))
...
dfs.fasta = dataframe2fas(dfs, file= paste(repository, "/0_data/EXPORT/fasta/CRISPRscope_185_spacers.fasta", sep=""))
...
dfs.fasta = dataframe2fas(dfs, file= paste(repository, "/0_data/EXPORT/fasta/CRISPRscope_185_spacers_BLAST.fasta", sep=""))
...
dfs.fasta = dataframe2fas(dfs, file=paste(repository, "/0_data/EXPORT/fasta/CRISPRscope_185_repeats.fasta", sep=""))
...
dfs.fasta = dataframe2fas(dfs, file=paste(repository, "/0_data/EXPORT/fasta/CRISPRscope_genomic_185_spacers.fasta", sep=""))
...
dfsr.fasta = dataframe2fas(dfsr, file=paste(repository, "/0_data/EXPORT/fasta/CRISPRscope_genomic_185_repeats.fasta", sep=""))
...
CRISPRscope_tbl_26 %>% 
  ...
  select(-Subtype) %>% 
  distinct() %>% 
  write.csv(file = paste(repository, "/0_data/EXPORT/SpeciesCasType.csv", sep=""), quote = T, row.names = F, col.names = T)
...
CRISPRscope_tbl_26 %>% 
  ...
  write.csv(file = paste(repository, "/0_data/EXPORT/StrainCasType.csv", sep=""), quote = T, row.names = F, col.names = T)
...
CRISPRscope_tbl_26 %>% select(Organism, Strain, ArrayID) %>%  distinct() %>% 
  ...
  write.csv(file = paste(repository, "/0_data/EXPORT/MeanArrayPerStrain.csv", sep=""))
...
CRISPRscope_tbl_26 %>% select(Organism, Strain, ArrayID) %>%  distinct() %>% 
  group_by(Strain) %>% mutate(nb_array_strain = n()) %>% select(Organism, Strain, nb_array_strain) %>% distinct() %>% 
  write.csv(file = paste(repository, "/0_data/EXPORT/NbArrayPerStrain.csv", sep=""))
...
bind_rows(a,b) %>% write.csv(file = paste(repository, "/0_data/EXPORT/nb_species_per_subtype_26_185.csv", sep = ""), row.names = F)
```
...
CRISPRscope_tbl_185 %>% 
  ...
  write.csv(file = paste(repository,"/0_data/EXPORT/Cas_subtypes_per_species_185.csv", sep=""), row.names = F)
```
...
#tmp_arrcount %>% write.csv(file = paste(repository,"/0_data/EXPORT/raw_org_arr_sp_class_type.csv", sep=""))
...
results_ani <- list.files(path = paste(repository, "/0_data/fastANI_results/MERGED/", sep=""))

for (ani in results_ani){
  print(ani)
  tmp <- read_delim(file = paste(repository, "/0_data/fastANI_results/MERGED/", ani, sep=""), 
                  "\t", escape_double = FALSE, 
...
ani_merged %>%
  write_csv(file = paste(repository, "/0_data/IMPORT_EXPORT/MERGED_ANI_output_clean.csv", sep=""))
```
...
```{r, eval = FALSE}
ani_merged <- read_csv(file = paste(repository, "/0_data/IMPORT_EXPORT/MERGED_ANI_output_clean.csv", sep=""))
...
ANI_merged_filtered %>% write.csv(file = paste(repository, "/0_data/IMPORT_EXPORT/ANI_merged_filtered.csv", sep=""), row.names=FALSE)
...
ANI_merged_filtered <- read_csv(file = paste(repository, "/0_data/IMPORT_EXPORT/ANI_merged_filtered.csv", sep=""))

# Function to get 
get_pair_shared_seq <- function(strain1str, strain2str, spacer=TRUE){

  if(spacer){
    strain1_seq <- CRISPRscope_tbl_185 %>% filter(Strain == strain1str) %>% select(cluster_spacer)
    strain1_seq <- strain1_seq$cluster_spacer

    strain2_seq <- CRISPRscope_tbl_185 %>% filter(Strain == strain2str) %>% select(cluster_spacer)
...
ANI_Shared_spacers_strain_pair %>% 
  write.csv(file = paste(repository, "/0_data/IMPORT_EXPORT/ANI_Shared_spacers_strain_pair.csv", sep=""), row.names=FALSE)
...
```{r fig.height=8, fig.width=15}
ANI_Shared_spacers_strain_pair <- read_csv(file = paste(repository, "/0_data/IMPORT_EXPORT/ANI_Shared_spacers_strain_pair.csv", sep="")) # 21 species
...
ANI_Shared_spacers_strain_pair_filtered %>% write.csv(file = paste(repository, "/0_data/IMPORT_EXPORT/ANI_data_plot.csv", sep=""))
...
ANI_Shared_spacers_strain_pair_filtered <- read_csv(file = paste(repository, "/0_data/IMPORT_EXPORT/ANI_data_plot.csv", sep="")) %>% 
...
# Function to get 
get_pair_nbof_shared_seq <- function(strain1str, strain2str, spacer=TRUE){
...
get_mean_nb_spacers <- function(strain1str, strain2str, spacer=TRUE){
...
ANI_Shared_numberof_spacers_strain_pair_raw <- ANI_merged_filtered %>% 
  mutate(nb_shared_spacers = future_map2(strain_query, strain_ref, spacer=TRUE, get_mean_nb_spacers))
  #mutate(shared_spacers = purrr::pmap(list(strain1, strain2, spacer=TRUE), get_pair_shared_seq)) 
...
ANI_Shared_numberof_spacers_strain_pair %>% 
  write.csv(file = paste(repository, "/0_data/IMPORT_EXPORT/ANI_Shared_numberof_spacers_strain_pair.csv", sep=""), row.names=FALSE)
...
plt_nb_repeat %>% write.csv(file = paste(repository, "/0_data/IMPORT_EXPORT/plt_nb_repeat.csv", sep=""), row.names=FALSE)

plt_nb_repeat = read_csv(file  = paste(repository, "/0_data/IMPORT_EXPORT/plt_nb_repeat.csv", sep=""))
...
plt_nb_array %>% write.csv(file = paste(repository, "/0_data/IMPORT_EXPORT/plt_nb_array.csv", sep=""), row.names=FALSE)

plt_nb_array = read_csv(file = paste(repository, "/0_data/IMPORT_EXPORT/plt_nb_array.csv", sep=""))
...
CRISPRscope_tbl_26 %>%
  ...
  write.csv(file = paste(repository,"/0_data/EXPORT/Lhelv_Ldelb_scaffolds.csv", row.names=FALSE)
...
sorted_results_1 <- read_table2(paste(repository, "/0_data/pairwise_alignment_results/sorted_results.csv", sep=""),
    col_names = c("count", "Editdistance")) %>%
  add_row(count = 677078, Editdistance = 0) %>% 
  arrange(Editdistance)

# Raw file for sd and mean
merged_slices <- read_csv(paste(repository, "/0_data/pairwise_alignment_results/merged_slices.csv", sep=""), 
    col_names = FALSE)
...
merged_slices_custom_long <- read_csv(paste(repository, "/0_data/pairwise_alignment_results/merged_slices_custom_1.csv", sep=""), 
    col_names = FALSE) %>% 
  add_row(X1 = pad_long)
library(readr)
merged_slices_custom_short <- read_csv(paste(repository, "/0_data/pairwise_alignment_results/merged_slices_custom_2.csv", sep=""), 
    col_names = FALSE) %>% 
...
sorted_results_long <- read_table2(paste(repository, "/0_data/pairwise_alignment_results/sorted_results_custom_long.csv", sep=""),
                                   col_names = c("count", "Editdistance")) %>%
  mutate(group = "long")%>% 
...
sorted_results_short <- read_table2(paste(repository, "/0_data/pairwise_alignment_results/sorted_results_custom_short.csv", sep=""), 
                                    col_names = c("count", "Editdistance")) %>%
  mutate(group = "short") %>% 
...
sorted_results_30 <- read_table2(paste(repository, "/0_data/pairwise_alignment_results/sorted_results_custom_30.csv", sep=""),
                                   col_names = c("count", "Editdistance")) %>%
  mutate(group = "30s") %>% 
...
sorted_results_not30 <- read_table2(paste(repository, "/0_data/pairwise_alignment_results/sorted_results_custom_not30.csv", sep=""), 
                                    col_names = c("count", "Editdistance")) %>%
  mutate(group = "not30") %>% 
...
merged_slices_custom_30_1 <- read_csv(paste(repository, "/0_data/pairwise_alignment_results/merged_slices_custom_30_1.csv", sep=""), 
    col_names = FALSE) %>% 
  add_row(X1 = pad_30)
library(readr)
merged_slices_custom_not30_2 <- read_csv(paste(repository, "/0_data/pairwise_alignment_results/merged_slices_custom_not30_2.csv", sep=""), 
    col_names = FALSE) %>% 
...
merged_slices_custom_dist6 <- read_csv(paste(repository, "/0_data/pairwise_alignment_results/merged_slices_custom_dist6.csv", sep=""), 
...
# Strain-strain ANI
ANI_Shared_spacers_strain_pair <- read_csv(paste(repository, "/0_data/IMPORT_EXPORT/ANI_Shared_spacers_strain_pair.csv", sep=""))
...
```{r}
# Strain-strain ANI
ANI_Shared_spacers_strain_pair <- read_csv(file=paste(repository, "/0_data/IMPORT_EXPORT/ANI_Shared_spacers_strain_pair.csv", sep=""))
...
# compute the complement and reverse complement

# seq.y.complement <- as.character(complement(DNAStringSet(test_pairs$seq.y)))
# seq.y.reverseComplement <- as.character(reverseComplement(DNAStringSet(test_pairs$seq.y)))

# Option: join complements and reverse complement to the dataset
#         problem: map takes 2 parameters only
# 
# test_pairs <- test_pairs %>% 
#   mutate(seq.y.complement = seq.y.complement) %>% 
#   mutate(seq.y.reverseComplement = seq.y.reverseComplement)
...
fwrite(data %>%
           mutate(dist = future_map2_dbl(seq.x, seq.y, pairwise_comparison, .progress = TRUE)) %>%
           unnest(dist),
         file=paste(repository, "/0_data/IMPORT_EXPORT/Random_editdistance_spacer_comparison.csv", sep = ""))


random_edit_distance_raw <- read_csv(file=paste(repository, "/0_data/IMPORT_EXPORT/Random_editdistance_spacer_comparison.csv", sep=""), col_names = T) 
