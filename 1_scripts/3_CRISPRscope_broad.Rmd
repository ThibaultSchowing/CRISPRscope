...
CRISPRblast_2 <- bind_rows(CRISPRblast_IMGVR_2, CRISPRblast_nt_2, CRISPRblast_PLSDB_2) %>% 
  ...
  mutate(final_hit = ifelse(cluster %in% CRISPR_hits_list, "CRISPR", "")) %>% # Assign CRISPR first.
...
CRISPRscope_blast_cluster_hit_type_2_completed <- bind_rows(CRISPRscope_blast_cluster_hit_type_2,
  ...
  mutate(final_hit = ifelse(final_hit == "CRISPR", "No hit", final_hit))
...
stermvotu <- votu_sterm_project_count %>% filter(ProjectID == "Pasolli et al. 2020" | ProjectID == "Swiss starter culture")%>% 
  mutate(fraction = 100*(total_distinct_sterm_votu/total_spacers_with_sterm_IMG_hit)) 
mean(stermvotu$fraction)
sd(stermvotu$fraction)
