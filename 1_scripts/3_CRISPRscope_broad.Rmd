...
CRISPRscope_tbl_26 <- readRDS(file = paste(repository, "/0_data/CRISPRscope_results/export_complete_SEL26.rds", sep=""))
CRISPRscope_tbl_185 <- readRDS(file = paste(repository, "/0_data/CRISPRscope_results/export_complete_SEL185.rds", sep=""))
...
S1_G185 %>% write.table(file=paste(repository, "/0_data/EXPORT/list_S1G185.csv", sep=""), row.names=FALSE, col.names = FALSE)
S2_G26 %>% write.table(file = paste(repository, "/0_data/EXPORT/list_S1G26.csv", sep=""), row.names=FALSE, col.names = FALSE)
S3_M %>% write.table(file = paste(repository, "/0_data/EXPORT/list_S3M.csv", sep=""), row.names=FALSE, col.names = FALSE)
...
R1_G185 %>% write.table(file = paste(repository, "/0_data/EXPORT/list_R1G185.csv", sep=""), row.names=FALSE, col.names = FALSE)
R2_G26 %>% write.table(file = paste(repository, "/0_data/EXPORT/list_R1G26.csv", sep=""), row.names=FALSE, col.names = FALSE)
R3_M %>% write.table(file = paste(repository, "/0_data/EXPORT/list_R3M.csv", sep=""), row.names=FALSE, col.names = FALSE)
```
...
dfr <- data.frame(outr$header, outr$seq)
dfr.fasta = dataframe2fas(dfr, file = paste(repository, "/0_data/EXPORT/fasta/CRISPRscope_ALL_REPEATS.fasta", sep=""))
...
dfs <- data.frame(out2$header, out2$seq)
dfs.fasta = dataframe2fas(dfs, file = paste(repository, "/0_data/EXPORT/fasta/CRISPRscope_ALL_SPACERS.fasta", sep=""))
...
Clusters_CRISPRscope_ALL_SPACERS <- read_csv(paste(repository, "/0_data/IMPORT/clustering/Clusters1_CRISPRscope_ALL_SPACERS.csv", sep=""))
...
dfs <- data.frame(out2$header, out2$seq)
dfs.fasta = dataframe2fas(dfs, file = paste(repository, "/0_data/EXPORT/fasta/cluster1_spacers_reference_CRISPRscope.fasta", sep=""))
...
dfs.fasta = dataframe2fas(dfs, file=paste(repository, "/0_data/EXPORT/fasta/RepeatsMetagenomeOnly.fasta", sep=""))
...
CRISPRscope_meta_tbl_filtered %>% select(Subtype, cluster_repeat_identity) %>% 
  ...
  write.csv(file=paste(repository, "/0_data/EXPORT/castype_cluster_metagenomeOnly.csv", sep=""), row.names = F, col.names = T) # 286
...
CRISPRscope_tbl_185 %>% select(Cas_subtype, cluster_repeat_identity) %>% 
  ...
  write.csv(file=paste(repository, "/0_data/EXPORT/castype_cluster_R2_G26.csv", sep=""), row.names = F, col.names = T) #113
...
CRISPRscope_tbl_185 %>% select(Cas_subtype, cluster_repeat_identity) %>% 
  ...
  write.csv(file=paste(repository, "/0_data/EXPORT/castype_cluster_R_G185_ONLY.csv", sep=""), row.names = F, col.names = T)#425
...
CRISPRblast_nt_2 <- read_delim(paste(repository, "/0_data/BLAST/CRISPRblast_nt_vincent.txt", sep=""), 
  ...
  # read_delim(paste(repository, "/0_data/BLAST/CRISPRblast_nt_vincent.txt", sep=""), 
  #     "\t", escape_double = FALSE, col_names = c("qseqid", "sseqid", "pident", "length", "mismatch", "gapopen", "qstart", "qend", 
  #                                                "sstart", "send", "evalue", "bitscore", "stitle", "slen", 
  #                                                "staxid", "ssciname", "sskingdom", "staxids"), trim_ws = TRUE) %>% 
  #   select(-staxid, -ssciname, -sskingdom, -staxids) %>% 
...
CRISPRscope_blast_cluster_hit_type_2_completed %>% filter(final_hit == "Bacterial") %>% 
  ...
  write_csv(file = paste(repository, "/0_data/EXPORT/BLAST_Bacterial_hits_target_id.csv", sep=""))
...
genomic_blast_hits_2 %>% write_csv(file = paste(repository, "/0_data/EXPORT/BLAST_genomic_plot.csv", sep=""), col_names = T)
metagenomic_blast_hits_2 %>% write_csv(file = paste(repository, "/0_data/EXPORT/BLAST_metagenomic_plot.csv", sep=""), col_names = T)
...
blast_meta_plot <- metagenomic_blast_hits_2  %>% 
  ...
  scale_fill_manual(values = c("grey","darkorchid","mediumorchid", "palevioletred3"))+  
...
```{r}
genomic_blast_hits_2 <- read_csv(file = paste(repository, "/0_data/EXPORT/BLAST_genomic_plot.csv", sep=""), col_names = T)
metagenomic_blast_hits_2 <- read_csv(file = paste(repository, "/0_data/EXPORT/BLAST_metagenomic_plot.csv", sep=""), col_names = T)
...
hit_plt_2$final_hit <- factor(hit_plt_2$final_hit, levels = c("No hit", "Viral", "Plasmid", "Bacterial"))
...
# Get failed attempts. 
# entrez_batch_fail_1 <- read_csv(paste(repository, "/0_data/IMPORT/Blast_nr_hits/entrez_batch_fail_1.txt", sep=""), col_names = FALSE) %>% 
...
save(bacterial_viral_protein_hits_list, file=paste(repository, "/0_data/IMPORT_EXPORT/bacterial_viral_protein_hits_list", sep=""))
...
```{r}
IMGVR_all_Sequence_information <- read_delim(paste(repository, "/0_data/DB/IMGVR/IMG_VR_2020-10-12_5.1/IMGVR_all_Sequence_information.tsv", sep=""), 
    "\t", escape_double = FALSE, trim_ws = TRUE)

IMGVR_all_Host_information <- read_delim(paste(repository, "/0_data/DB/IMGVR/IMG_VR_2020-10-12_5.1/IMGVR_all_Host_information.tsv", sep=""), 
    "\t", escape_double = FALSE, trim_ws = TRUE)
...
write.csv(joined_IMGVR, file = paste(repository, "/0_data/EXPORT/IMGVR_Hits_join.csv", sep=""), row.names = F)
...
```{r}
joined_IMGVR <- read.csv( file = paste(repository, "/0_data/EXPORT/IMGVR_Hits_join.csv", sep=""))
```
...
```{r}
library(patchwork)
# num_of_vOTUs_per_taxon <- read_csv(paste(repository, "/0_data/IMPORT/IMG_Hits/num_of_vOTUs_per_taxon.csv", sep = ""))

# UViG_match_table <- read_csv(paste(repository, "/0_data/IMPORT/IMG_Hits/UViG_match_table.csv", sep = ""))
...
CRISPRscope_tbl_26 %>% select(Organism, Strain, cluster_spacer_identity) %>% 
  ...
  write_csv(file = paste(repository, "/0_data/EXPORT/fraction_votu_per_strain_duplicates.csv", sep=""), col_names = T)
...
vOTUS_in_which_species <- read_csv(paste(repository, "/0_data/IMPORT/IMG_Hits/vOTUS_in_which_species.csv", sep=""))
...
fraction_votu_per_species %>% write_csv(file = paste(repository, "/0_data/EXPORT/fraction_votu_per_species.csv", sep=""), col_names = T)
...
fraction_votu_per_strain %>% write_csv(file = paste(repository, "/0_data/EXPORT/fraction_votu_per_strain.csv", sep=""), col_names = T)
```
...
fraction_votu_per_species <- read_csv(file = paste(repository, "/0_data/EXPORT/fraction_votu_per_species.csv", sep=""), col_names = T)
fraction_votu_per_strain <- read_csv(file = paste(repository, "/0_data/EXPORT/fraction_votu_per_strain.csv", sep=""), col_names = T)
...
megagenomic_project_sample_votu %>% mutate(sterm = ifelse(vOTU %in% votu_in_which_species_sterm$vOTU, "sterm", "unknown")) %>% 
  ...
  write_csv(file = paste(repository, "/0_data/EXPORT/VOTUs_metagenomes_per_sample.csv", sep=""))
  # To have 'per project' add %>% select(ProjectID, total_sterm_in_imgvr, total_votu_sterm_project, fraction_project) %>% distinct()

metagenomic_votu_sterm = read_csv(file = paste(repository, "/0_data/EXPORT/VOTUs_metagenomes_per_sample.csv", sep=""))
```
...
CRISPRscope_meta_tbl_filtered %>% select(ProjectID, SRA_ID, cluster_spacer_identity) %>% 
  ...
  renameSamples() %>% 
  left_join(joined_IMGVR %>% select(vOTU, Taxonomic.classification) %>% distinct(), by=c("vOTU")) %>% 
  distinct() %>% write_csv(file = paste(repository, "/0_data/EXPORT/vOTU_per_sample.csv", sep=""))
...
```{r}
CRISPR_spacer_coverage %>% read_csv(file = paste(repository, "/0_data/IMPORT_EXPORT/DR_SP_DR_results.csv", sep=""), col_names = T)
...
```{r}
CRISPR_spacer_coverage_2 <- CRISPR_spacer_coverage %>% read_csv(file = paste(repository, "/0_data/IMPORT_EXPORT/DR_SP_DR_results.csv", sep=""), col_names = T) %>% 
...
# Raw file 
merged_slices <- read_csv(file=paste(repository, "/0_data/pairwise_alignment_results/merged_slices.csv", sep=""), 
    col_names = FALSE)
